...
class ScanData:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.disable_interp_flag:
            self.interp_num = 500
            self.grid_x, self.grid_y, self.grid_z = self.create_grid(self.interp_num) # type: ignore
            return
    ...
    def display_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            'peak_location': self.peak_location(), # type: ignore
            'peak_cup_current': self.peak_cup_current() # type: ignore
            * 1e9,
            'peak_total_current': self.peak_total_current() # type: ignore
            * 1e6,  # returns value in microamps, # type: ignore
            'FWHM_area': self.fwhm_area(), # type: ignore
            'FWQM_area': self.fwqm_area(), # type: ignore
            'solenoid_current': self.solenoid_current,
            'test_stand': self.test_stand,
            'centroid': self.compute_weighted_centroid(), # type: ignore
            'FWHM_min_diam': self.fwhm_diams()['min'], # type: ignore
            'FWHM_max_diam': self.fwhm_diams()['max'], # type: ignore
            'FWQM_min_diam': self.fwqm_diams()['min'], # type: ignore
            'FWQM_max_diam': self.fwqm_diams()['max'], # type: ignore
        }
