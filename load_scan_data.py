    ...
    def load_scan_data(self, filepath: str) -> ScanData:
        ...
        if not csv_loaded:
            try:
                ...
                df: pd.DataFrame = pd.read_csv(filepath, header=None, usecols=[0], nrows=7, skiprows=[3,7])
            
                scan_datetime_data: str = str(df.iloc[0,0])
                if scan_datetime_data.split()[0] != '#DATE/TIME:':
                    raise Exception('CSV is not a beam scan csv.')
                scan_datetime: str = scan_datetime_data.replace('#DATE/TIME: ','').strip()
                ...
                step_size: float = float(step_size_data.replace('#Step Size(mm): ','').strip()) # mm

                beam_voltage_data: str = str(df.iloc[3,0])
                beam_voltage: float = round(int(beam_voltage_data.replace('#Beam Voltage: ','').strip()),-2) * 1e-3 # kV
                if beam_voltage.is_integer():
                    beam_voltage = int(beam_voltage)

                extractor_voltage_data: str = str(df.iloc[4,0])
                extractor_voltage: float = round(int(extractor_voltage_data.replace('#Extractor Voltage: ', '').strip()), -2) * 1e-3 # kV
                if extractor_voltage.is_integer():
                    extractor_voltage = int(extractor_voltage)

                beam_supply_current_data: str = str(df.iloc[5,0])
                beam_supply_current: str = beam_supply_current_data.replace('#Beam Supply Current: ','').strip() # uA

                pressure_data: str = str(df.iloc[6,0])
                pressure: str = pressure_data.replace('#Chamber Pressure:  ','') # mBar
                ...
                df: pd.DataFrame = pd.read_csv(filepath, usecols=[0, 1, 2, 3], skiprows=9)
        ...
        resolutions: dict[float,str] = {0.221: 'Highest', 0.442: 'High', 0.884: 'Med', 1.767: 'Low'}
