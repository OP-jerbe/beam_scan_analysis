...
def surface(self, grid_x, grid_y, grid_z, centroid: tuple[float, float], x_slice: DataFrame, y_slice: DataFrame) -> go.Figure:
    ...
    if any(value is not None for value in self.z_scale):
        if contour_size > 0:
            fig = go.Figure(data=[go.Surface(
                    ...
                    opacity=0.8,
                    contours=dict(
                        x={'show': True, 'start': grid_x.min(), 'end': grid_x.max(), 'size': int(1000), 'color': 'gray'},
                        y={'show': True, 'start': grid_y.min(), 'end': grid_y.max(), 'size': int(1000), 'color': 'gray'},
                        z={'show': True, 'start': self.levels[0], 'end': self.levels[1], 'size': (self.levels[1]-self.levels[0]) - 1e-9,'color': 'red'} 
                    )
            )])
        ...
        fig.add_trace(go.Scatter3d(x=x_slice['X Coordinate'],
                                   y=grid_y[0,:],
        ...
        fig.add_trace(go.Scatter3d(x=grid_x[:,0],
        ...
        fig.add_trace(go.Scatter3d(x=[centroid[0]] * len(y_slice),
                                   y=y_slice['Y Coordinate'],
                                   z=y_slice['Faraday Cup Current'],
                                   mode='lines', 
                                   line=dict(color='black', width=2),
        ...
        fig.update_layout(
                    ...
                    scene=dict(zaxis=dict(
                        range=[self.z_scale[0], self.z_scale[1]]
                    )),
                    scene_camera=dict(
